...
public class MobLavaBeam : Spell {
	...
	void Start () {
        caster.GetComponent<RigidbodyWrapper>().EraseExternalForcesForTime(_lifetime + 1);
    ...
    private IEnumerator ManageAndDestroy(float delay)
    {
        ...
        float timeCounter = 0;
        float lineStartSize = 1.4f; // value knows from the inspector
    ...
    void OnTriggerStay(Collider col)
    {
        ...
        if(lv != null && GameDictionary.AreEnemies(caster.tag, col.gameObject.tag))
        {
